...
{
    DeviceAttributes dev_attrs = { "noname", "noserial", "nosize", "noblocksize", "nopath" };
    int res;
    const char *syspath;
    ...
    res = sd_device_get_property_value(dev, "ID_FS_BLOCKSIZE", &block_size);
    if (res >= 0)
        dev_attrs.block_size = (char *)block_size;
    else if (sd_device_get_property_value(dev, "ID_PART_TABLE_TYPE:", &block_size) >= 0)
        dev_attrs.block_size = (char *)block_size;
...
int device_event_handler(sd_device_monitor *monitor, sd_device *device, void *userdata)
{
    ...
    strlen(dev_attrs.uuid) + strlen(dev_attrs.block_size) + 3;
    device_uid = (char *)xmalloc(device_uid_length);
...
int setup_udev_monitoring(cubby_opts_t *opts)
{
    ...
    if (r < 0)
        die("Could not create new sd device monitor");

    r = sd_device_monitor_filter_add_match_subsystem_devtype(monitor, SUBSYSTEM, DEVICE_TYPE);
    if (r < 0)
        die("Could not add subsystem match to monitor");
...
DeviceList new_device_list()
{
    ...
    char **devices      = xmalloc(sizeof(*devices) * dev_buffer_size);
    ...
    for (current_device = sd_device_enumerator_get_device_first(sd_dev_enum);
        ...
        if (i >= dev_buffer_size) {
            dev_buffer_size *= 2;
            char **new_devices = xrealloc(devices, sizeof(*new_devices) * dev_buffer_size);
            devices = new_devices;
        }
